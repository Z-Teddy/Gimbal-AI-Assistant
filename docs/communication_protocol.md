...
- 到当前 v4.0 阶段，协议层实际承担了哪些最小模式闭环能力

当前协议仍然是单向主链路，主要服务于视觉云台最小闭环，不是完整的双向控制总线，也没有 ACK、序列重传或复杂错误恢复设计。

## 2. 当前阶段能力变化

相对早期仅发送坐标的版本，当前 v4.0 在协议层已经具备最小状态化与 mode 闭环能力：
...
- v4.0 新增的语音输入没有引入新的串口协议命令字
- 语音命令在 RK3588 主线程内被转换为既有 mode / camera 行为
- 对 STM32 侧来说，语音触发的 `TRACK / HOLD / SCAN / RETURN_HOME` 仍然表现为既有 `CMD_SET_MODE`
...
因此默认 `default.yaml` 仍尽量接近安全基础配置；如需启用成熟闭环链路，建议使用 `configs/runtime_tracking.yaml`，如需同时启用语音输入，建议使用 `configs/runtime_voice.yaml`。
